...
function generateFolderContents(folderPath, files) {
  return `
    <!DOCTYPE html>
    <html lang="en">
    <head>
    ...
    <body class="bg-gray-100 min-h-screen">
      <div id="main-container" class="container mx-auto px-4 py-8">
        <div class="flex justify-between items-center mb-8">
          <h1 class="text-4xl font-bold">Folder Contents</h1>
            ...
            ${files
              ...
              <div class="bg-white rounded-lg shadow-md p-4 hover:shadow-lg transition-shadow duration-300">
                ...
                <div class="flex space-x-2">
                  <button class="bg-green-500 hover:bg-green-600 text-white font-bold py-1 px-3 rounded text-sm" onclick="loadFile('${folderPath}', '${file}')">
                    Play
                  </button>
                  <a href="${path.join(folderPath, file)}" target="_blank" class="bg-blue-500 hover:bg-blue-600 text-white font-bold py-1 px-3 rounded text-sm">
                    Open
                  </a>
                </div>
              </div>
...
app.get("/", (req, res) => {
  ...
  let gameFiles = { files: [], totalPages: 0 };
  let toolsFiles = { files: [], totalPages: 0 };

  if (filter === "all" || filter === "games") {
    gameFiles = getFileList(gameFolder, itemsPerPage, currentPage);
  }

  if (filter === "all" || filter === "tools") {
    toolsFiles = getFileList(toolsFolder, itemsPerPage, currentPage);
  }
              ...
              : ""
                  ...
                  <div id="pagination" class="mt-8 flex justify-center flex-wrap">
                    ${Array.from(
                      {
                        length: Math.max(
                          gameFiles.totalPages,
                          toolsFiles.totalPages,
                        ),
                      },
                      (_, i) => i + 1,
                    )
                      .map(
                        (page) => `
                          <a href="/?items=${itemsPerPage}&page=${page}&filter=${filter}" class="px-3 py-2 m-1 ${
                            currentPage === page
                ...
                <script>
                  function changeItemsPerPage() {
                    ...
                    window.location.href = '/?items=' + select.value + '&filter=' + filterSelect.value;
                  }
...
app.get("/game", (req, res) => {
  ...
  const html = generateFolderContents("/game", files);
  res.send(html);
});
...
app.get("/tools", (req, res) => {
  ...
  const html = generateFolderContents("/tools", files);
  res.send(html);
});
